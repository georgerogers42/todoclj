  ...
  (fn [req]
    (let [x (atom nil)]
      (try
        (reset! x (f req))
        (catch Exception e
          (reset! x (assoc (redirect "/") :flash (str e)))))
      @x)))
(def db
  (merge
    ...
    (fn [req]
      (let [todos (sql/with-connection db
                    (sql/with-query-results res
                      [(str "select c.id, t.id as tid, t.task, t.done,"
                            " c.name from"
                            " Todos t"
                ...
                [:body
                 ...
                 (for [category categories]
                   [:div
                    [:p (category 1)]
                    [:form {:method "post"
                            :action (str "/category/" (category 0) "/delete")}
